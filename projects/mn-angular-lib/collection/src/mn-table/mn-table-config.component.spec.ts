...
describe('MnTable data source configuration', () => {
  ...
  it('shows the translated header of a column added after init', () => {
    ...
    const header: HTMLTableCellElement = fixture.nativeElement.querySelector(
      'thead th[data-column-key="image"]',
    );
  ...
  for (const selectionMode of ['single', 'multi'] as const) {
    it(`names the selection column header in ${selectionMode}-select mode`, () => {
      ...
      const selectionHeader: HTMLTableCellElement = fixture.nativeElement.querySelector(
        'thead tr:first-child th:not([data-column-key])',
      );
      expect(selectionHeader)
        .withContext('the selection column should render a header')
        .toBeTruthy();
  ...
  it('announces loading through a status region and marks the body busy (table)', () => {
    fixture.componentRef.setInput(
      'dataSource',
      makeDataSource({ state: MnCollectionState.LOADING }),
    );
    fixture.detectChanges();
    ...
    fixture.componentRef.setInput(
      'dataSource',
      makeDataSource({ state: MnCollectionState.RETRIEVED }),
    );
    fixture.detectChanges();
    expect(fixture.nativeElement.querySelector('[role="status"]').textContent?.trim()).toBe('');
    expect(
      fixture.nativeElement.querySelector('[role="region"]').hasAttribute('aria-busy'),
    ).toBeFalse();
  });
});
