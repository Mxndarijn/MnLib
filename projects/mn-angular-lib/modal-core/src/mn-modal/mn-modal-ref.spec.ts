...
describe('MnModalRef', () => {
  ...
  it('closes cleanly before a shell is attached, and records the close so none gets mounted', (done) => {
    const pending = new MnModalRef<string>(null, {kind: ModalKind.FORM} as BaseModalConfig<string>);
    pending.afterClosed$.subscribe((event) => {
      expect(event.reason).toBe(ModalCloseReason.CANCELLED);
      expect(pending.isCloseRequested).toBeTrue();
      expect(pending.component).toBeUndefined();
      done();
    });
    expect(() => pending.update({})).not.toThrow();
    pending.dismiss(ModalCloseReason.CANCELLED);
  });
