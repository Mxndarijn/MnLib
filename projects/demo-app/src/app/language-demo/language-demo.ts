    ...
    >
      ...
      <app-demo-example [title]="'demo.switch_locale' | mnTranslate">
        ...
        <div class="button-row">
          <button (click)="switchLocale('en')" [class.active]="lang.locale === 'en'">
            English
          </button>
          <button (click)="switchLocale('nl')" [class.active]="lang.locale === 'nl'">
            Nederlands
          </button>
        </div>
      </app-demo-example>
      ...
      <!-- 1. Pipe usage -->
      <app-demo-example
        [title]="'demo.pipe.title' | mnTranslate"
        [hint]="'demo.pipe.description' | mnTranslate"
      >
        <div class="example">
          <h3>Template</h3>
      ...
      <app-demo-example
        [title]="'demo.standalone.title' | mnTranslate"
        [hint]="'demo.standalone.description' | mnTranslate"
      >
        <div class="example">
          <h3>Code</h3>
      ...
      <app-demo-example
        [title]="'demo.config.title' | mnTranslate"
        [hint]="'demo.config.description' | mnTranslate"
      >
        <div class="example">
          <h3>Config value</h3>
          <code
            >label: {{ '{' }} $translate: "demo.greeting", params: {{ '{' }} name: "Config"
            {{ '}' }} {{ '}' }}</code
          >
          <h3>Resolved</h3>
      ...
      <app-demo-example
        [title]="'demo.reactive_config.title' | mnTranslate"
        [hint]="'demo.reactive_config.description' | mnTranslate"
      >
        <div class="example">
          <h3>mn-config.json5</h3>
          <code>title: {{ '{' }} $translate: "demo.reactive_config.title" {{ '}' }}</code
          ><br />
          <code
            >description: {{ '{' }} $translate: "demo.reactive_config.description" {{ '}' }}</code
          ><br />
          <code
            >label: {{ '{' }} $translate: "demo.reactive_config.label", params: {{ '{' }} name:
            "World" {{ '}' }} {{ '}' }}</code
          >
      ...
      <app-demo-example
        [title]="'demo.form.section_title' | mnTranslate"
        [hint]="'demo.form.section_description' | mnTranslate"
      >
        <div class="example">
          <h3>mn-config.json5</h3>
          <code>placeholder: {{ '{' }} $translate: "demo.form.name.placeholder" {{ '}' }}</code
          ><br />
          <code>label: {{ '{' }} $translate: "demo.form.name.label" {{ '}' }}</code>
          <h3>mn-input-field (switch locale to see placeholder/label update)</h3>
          <mn-lib-input-field
            [formControl]="nameCtrl"
            [props]="{ id: 'demo-name', type: 'text', size: 'md', borderRadius: 'md' }"
          ></mn-lib-input-field>
        </div>
        <div class="example">
          ...
          <mn-lib-textarea
            ...
            [props]="{ id: 'demo-message', size: 'md', borderRadius: 'md', rows: 3 }"
          ></mn-lib-textarea>
        </div>
      </app-demo-example>
      ...
      <app-demo-example
        [title]="'demo.validation.section_title' | mnTranslate"
        [hint]="'demo.validation.section_description' | mnTranslate"
      >
        <div class="example">
          <h3>mn-config.json5</h3>
          <code
            >errorMessages: {{ '{' }} required: {{ '{' }} $translate: "validation.required"
            {{ '}' }} {{ '}' }}</code
          >
          <h3>mn-input-field (required) — touch and clear to see translated error</h3>
          <mn-lib-input-field
            [formControl]="valNameCtrl"
            [props]="{ id: 'validation-name', type: 'text', size: 'md', borderRadius: 'md' }"
          ></mn-lib-input-field>
        </div>
        <div class="example">
          <h3>mn-input-field (required + email) — enter invalid email to see translated error</h3>
          <mn-lib-input-field
            [formControl]="valEmailCtrl"
            [props]="{ id: 'validation-email', type: 'email', size: 'md', borderRadius: 'md' }"
          ></mn-lib-input-field>
        </div>
        <div class="example">
          <h3>mn-textarea (required) — touch and clear to see translated error</h3>
          <mn-lib-textarea
            [formControl]="valMessageCtrl"
            [props]="{ id: 'validation-message', size: 'md', borderRadius: 'md', rows: 3 }"
          ></mn-lib-textarea>
        </div>
      </app-demo-example>
...
export class LanguageDemo {
  ...
  async switchLocale(locale: string) {
    ...
    this.refresh();
    // The click's own notification already rendered the pre-await values; nothing marks this
    // view for what `refresh()` writes on the other side of the await.
